...
if($message<>""){
//$tmn="TMN EOC090000105-1016";
$TMN = substr("$message", 0, 3);
$rest = substr("$message", 4);

if($TMN=="tmn"){

   $check = "SELECT * FROM tblmember  WHERE  MemberID = '$rest'";
...
if ($num1 == "") {
   $putmessage= "TMN ตรวจสอบแล้วไม่พบข้อมูล :"."\n"."$message"; 
   $arrayPostData['to'] = $id;
   $arrayPostData['messages'][0]['type'] = "text";
   $arrayPostData['messages'][0]['text'] = $putmessage;
   pushMsg($arrayHeader,$arrayPostData);
